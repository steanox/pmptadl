    ...
    <div class="page-content-wrapper">
        <div class="page-content">
            ...
            <div class="row ">
                ...
                <div class="col-md-3">
                    <a href="{{Request::url()}}/upload-document" class="btn red"><i class="fa fa-download"></i> Download
                        All Files</a>
                </div>
            </div>
                ...
                <table class="table table-striped table-bordered table-hover" id="sample_1">
                    ...
                    @foreach($documents as $key=>$document)
                        ...
                        <tr class="odd gradeX">
                            ...
                            <td>
                                ...
                                <ul id="dropdown-file{{$key}}" class="dropdown-file">
                                    @for ($i = 1; $i < count($document["file"]);$i ++)
                                        <li>
                                            <a href="{{route('downloadSingleFile',['project_name'=> $projectName,'fileName'=>$document["file"][$i]->fileName])}}">Revision {{$i}}
                                                - {{$document["file"][$i]->fileName}}</a></li>
                                    @endfor
                            ...
                            <td>
                                <a href="#">
                                    <button type="button" class="btn blue" title="Download latest File"><i
                                                class="fa fa-download"></i></button>
                                </a>
                                @if ($document["status"] == "Approve")

                                    <a href="{{route("approveDocument",['project_name'=>$projectName,'discipline_id'=>$disciplineID,"documentID" => $document["documentID"],"documentFile_id" => $document["file"][0]->id])}}">
                                        <button type="button" class="btn green" title="Approve Project"><i
                                                    class="fa fa-check"></i></button>
                                    </a>
                                @endif
                                    <a href="{{ route("reviewDocument",['project_name'=>$projectName,'discipline_id' => $disciplineID,"document_id"=> $document["documentID"],"documentFile_id" => $document["file"][0]->id] )}}">
                                        <button type="button" class="btn yellow" title="Review Document"><i
